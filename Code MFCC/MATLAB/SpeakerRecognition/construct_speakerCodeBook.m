...
%     end
    ...
    C = mean(single(metrics{i}),1);%get the first codeword, which is the centroid of all training vectors the specific speaker
        ...
        while(dist_conv > dist_esp && iter < iter_limit)    
            ...
            for j = 1:vector_nb
                ...
                for k = 1:m
                    ...
                    C_min_temp = euclideanDistance(single(metrics{i}(j,:)), C(k,:));%find the power2 euclidien distance between training vector and codeword
            ...
            D = distortion(single(metrics{i}), C);
